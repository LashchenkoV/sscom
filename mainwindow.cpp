...
void MainWindow::init()
{
    ...
    this->setWindowTitle("SSCOM Linux Rus 0.4"); //
    ...
    mNetAddrLabel->setText("Viktor L.");
...
void MainWindow::currentIndexChanged()
{
    ...
    if(currentSettings.sendStringCache == "" ||
        ...
        /* 将小写换成大写 */
        int pos = ui->sendLineEdit->cursorPosition();
        ui->sendLineEdit->setText(ui->sendLineEdit->text().toUpper());
        ui->sendLineEdit->setCursorPosition(pos);
...
{
    if(serial->isOpen())
        currentSettings.stringStatus = "Открыт";
    else
        currentSettings.stringStatus = "Закрыт";
...
bool MainWindow::openSerialPort()
{
    ...
    if (serial->open(QIODevice::ReadWrite)) {
        if (setParameter(serial, p)) {
            ...
            // 2.更新文字
            mOpenSerialButton->setText("Закрыть порт");
            // 3.使能发送按键
...
void MainWindow::closeSerialPort()
{
    mLedLabel->setPixmap(QPixmap(":/led/off"));
    // 2.更新文字
    mOpenSerialButton->setText("Открыть порт");
    // 3.使能发送按键
    mSendFileButton->setDisabled(true);
    // 4.使能发送文件按键
    mSendButton->setDisabled(true);
